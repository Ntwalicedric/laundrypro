 ...
 * Uses Meta's WhatsApp Cloud API v18.0
 */
...
/**
 * Format pickup order message for dry cleaner
 */
...
export async function sendWhatsAppMessage(
  ...
  try {
    ...
    // The API requires digits only, no + prefix even if Meta UI shows it with +
    let phoneNumber = to.replace(/[\s+\-()]/g, "").trim();

    // Validate phone number format (10-15 digits, international format)
    ...
    // Try sending with digits-only format first (standard WhatsApp API format)
    ...
    // If "not in allowed list" error, try with + prefix as Meta might store it that way
    if (!response.ok && data.error && 
        (data.error.message?.includes("not in allowed list") || data.error.code === 131031)) {
      // Retry with + prefix (even though API docs say digits-only, Meta might match on this)
      response = await fetch(config.baseUrl, {
        ...
        body: JSON.stringify({
          messaging_product: "whatsapp",
          recipient_type: "individual",
          to: `+${phoneNumber}`, // Try with + prefix
          type: "text",
          text: {
            preview_url: false,
            body: message,
    ...
    if (!response.ok) {
      ...
      if (errorMessage.includes("not in allowed list") || errorCode === 131031) {
        // Meta might be strict about the format - try to provide both possible formats
 ...
 * Handles local format (0792875310) and converts to international (250792875310)
 */
function normalizePhoneNumber(phone: string, defaultCountryCode: string = "250"): string {
  // Remove all non-digit characters
  let cleaned = phone.replace(/[\s+\-()]/g, "").trim();
  
  // If number starts with 0 and is 10 digits, remove the leading 0 and add country code
  if (cleaned.startsWith("0") && cleaned.length === 10) {
    cleaned = defaultCountryCode + cleaned.substring(1);
  }
  
  // If number is 9 digits (without leading 0), add country code
...
/**
 * Send pickup order notification to dry cleaner
 */
export async function sendPickupOrderToDryCleaner(
  ...
  if (!dryCleanerPhone) {
    // Debug: log all environment variables related to WhatsApp
  ...
  // Normalize the phone number (handle local format like 0792875310)
